...
为了理解 NGF 的必要性，我们首先摒弃晦涩的术语，仅使用本科阶段的**线性代数**与**微积分**进行直观推导。其核心矛盾在于：**不同坐标系下的向量不能直接相加减。**

### 2.1 核心矛盾：坐标系的“汇率”

想象神经网络中的两个神经元 $A$ 和 $B$：
*   $A$ 位于坐标系 $S_A$ 中（例如，图片左上角）。
*   $B$ 位于坐标系 $S_B$ 中（例如，图片右下角，且相对于 $A$ 发生了旋转）。

**传统神经网络的错误**：直接计算 $x_A + x_B$。
这就像直接把“100 日元”加到“1 美元”上，得到“101”——这是没有物理意义的，因为单位（基底）不同。在几何变换（如旋转）下，这种直接相加会导致特征发生不可预测的畸变。

**NGF 的解决方案**：引入**汇率 (Exchange Rate)**，即**联络算子 (Connection)** $U_{A \leftarrow B}$。
在相加之前，必须先将 $x_B$ 转换到 $A$ 的坐标系中：
$$ x_{total} = x_A + U_{A \leftarrow B} \cdot x_B $$
其中 $U_{A \leftarrow B}$ 是一个正交旋转矩阵（或复数相位因子）。
...
但如果空间是弯曲的，或者坐标系是旋转的，$f(x+\Delta x)$ 和 $f(x)$ 处于不同的基底中，直接相减是非法的。我们需要先将 $f(x)$ **平行移动 (Parallel Transport)** 到 $x+\Delta x$ 处。

定义平行移动矩阵 $U(x+\Delta x, x) \approx I - i \mathbf{A}(x) \Delta x$，其中 $\mathbf{A}(x)$ 是我们要学习的**规范势 (Gauge Potential)**。

**协变导数 (Covariant Derivative)** 定义为：
$$ Df(x) = \lim_{\Delta x \to 0} \frac{f(x+\Delta x) - U(x+\Delta x, x)f(x)}{\Delta x} $$
代入近似公式：
$$ Df(x) \approx \lim_{\Delta x \to 0} \frac{f(x+\Delta x) - (I - i \mathbf{A} \Delta x)f(x)}{\Delta x} $$
$$ Df(x) = \frac{df}{dx} + i \mathbf{A}(x) f(x) $$

**结论**：所谓的“规范场”，本质上就是对普通导数的一个**线性修正项** $i\mathbf{A}(x)$。它抵消了坐标系变化带来的虚假变动，保留了真实的物理变化。
...
*   特征截面 $\Psi: M \to E$，满足 $\pi(\Psi(x)) = x$。这正是我们在 PyTorch 中操作的张量 `tensor[batch, channel, height, width]`。

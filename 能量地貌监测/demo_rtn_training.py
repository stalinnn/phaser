...
sys.path.append("/phaser/Energy_Landscape_Monitor")
from hessian_tracker import HessianTracker
from lie_bracket_tracker import LieBracketTracker
...
class RTNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if gate.sum() > 0:
            ...
            self.last_attn_weights = None # 简化，如果需要可以保存
        else:
            x_mixed = x_mamba
            self.last_attn_weights = None
            
        # 保存隐藏状态供探针 C 观察
        self.last_hidden_states = x_mixed
...
def train_rtn_demo():
    ...
    # 实例化 Hessian Tracker (探针 A)
    tracker = HessianTracker(model, max_iters=5, tol=1e-3, device=device)
    
    # 实例化 Lie Bracket Tracker (探针 B)
    # 我们监测 第0层 Mamba 序列混合模块 (代表信息流动) 和 热力学熵门模块 (代表拓扑相变控制) 之间的纠缠度
    lie_tracker = LieBracketTracker(model.layers[0].mamba, model.layers[0].thermo_gate, device=device)
    
    # 实例化 Entropy Detector (探针 C)
    entropy_radar = EntropyDetector()
    ...
    for step in range(201):
        ...
        # 实时探测能量景观曲率 (计算 Hessian 最大特征值)
        lambda_max = 0.0
        if step % 20 == 0:
            lambda_max = tracker.compute_max_eigenvalue(total_loss)
            
        total_loss.backward()
        
        # 计算探针 B (在梯度下降之前，因为需要访问梯度)
        lie_intensity = 0.0
        if step % 20 == 0:
            lie_intensity = lie_tracker.compute_lie_bracket_intensity()
        ...
        if step % 20 == 0:
            # 运行探针 C 观察磁畴粗糙度
            with torch.no_grad():
                # 我们观察最后一层的隐藏状态粗糙度 (取 Batch 的第一个样本)
                hidden_states = model.layers[-1].last_hidden_states
                if hidden_states is not None:
                    domain_info = entropy_radar.analyze_domain_walls(hidden_states)
                    # 计算有多少个显著的相变点 (畴壁)
                    num_walls = domain_info['domain_walls'][0].sum().item()
                    max_roughness = domain_info['roughness'][0].max().item()
                else:
                    num_walls = 0
                    max_roughness = 0.0
                    
            print(f"Step {step:03d} | Task Loss: {task_loss.item():.4f} | Gate Usage: {avg_gate_activation.item():.2%} | Hessian Max Eval: {lambda_max:.4f} | Lie Bracket Intensity: {lie_intensity:.4e} | Info Domain Walls: {num_walls} (Max Roughness: {max_roughness:.2f})")

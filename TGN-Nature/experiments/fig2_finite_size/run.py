...
def main():
    ...
    # Nature 1-column width = 88mm = 3.46 inches
    fig_width = 88 / 25.4
    fig_height = 65 / 25.4
    ...
    ax.errorbar(sizes, results['scalar_mean'], yerr=results['scalar_std'], 
               fmt='o--', color='#e74c3c', label='Scalar (Local)', 
               capsize=2, linewidth=1, markersize=3, elinewidth=0.5, markeredgewidth=0.5)
    
    # Tensor Curve
    ax.errorbar(sizes, results['tensor_mean'], yerr=results['tensor_std'], 
               fmt='s-', color='#2ecc71', label='Tensor (Distributed)', 
               capsize=2, linewidth=1, markersize=3, elinewidth=0.5, markeredgewidth=0.5)
    
    # Theoretical Scaling Lines
    n_arr = np.array(sizes)
    scale_factor = results['scalar_mean'][0] / (n_arr[0]**1.5) 
    ax.plot(n_arr, scale_factor * (n_arr**1.5), 'r:', alpha=0.5, 
            label='Theory: $\sim N^{1.5}$', linewidth=1)

    ax.set_xscale('log')
    ax.set_yscale('log') 
    
    ax.set_xlabel('System Size $N$', fontsize=7)
    ax.set_ylabel('Coordination Error (MSE)', fontsize=7)
    # ax.set_title('Overcoming Complexity Trap', fontsize=7) # Removed for Nature style
    ...
    plt.savefig('figures/finite_size_scaling_ring.pdf', format='pdf', dpi=1200)
    plt.savefig('figures/finite_size_scaling_ring.png', dpi=300)
    print("Saved results to figures/finite_size_scaling_ring.pdf and .png")

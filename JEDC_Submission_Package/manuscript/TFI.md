...
### 1.1 文献与贡献定位 (Literature Review and Contributions)

在系统性风险测度领域，学术界已发展出丰富的范式：从基于线性协方差的 PCA 与 Absorption Ratio (Kritzman et al., 2010)，到基于分位数回归的 CoVaR (Adrian & Brunnermeier, 2016)，再到基于格兰杰因果的网络连通性指标 (Billio et al., 2012)。然而，这些指标多依赖于长窗历史收益率，对突发性的微观流动性冲击反应滞后。与此同时，经济物理学 (Econophysics) 虽然在微观主体建模 (如 Lux & Marchesi, 1999) 和自旋玻璃动力学 (Bouchaud, 2013) 方面深刻解释了“崩溃相变”的成因，但其理论参数往往难以直接映射为实盘可跟踪的宏观因子。

本文的学术贡献恰在于连接了这两大孤岛：**我们首次将前沿的机器学习算子（Transformer 核心机制）从“黑盒预测器”转变为“白盒统计力学测量尺”**。在不预设任何复杂网络先验的前提下，本文从高频价格序列中逆向求解出刻画微观拥挤度的热力学参量，为应对非线性流动性冲击提供了一个可操作的预警指标。
...
### 2.2 绝热近似与 TFI 的数学定义 (Adiabatic Approximation and Mathematical Definition of TFI)

为了从高噪的日频收益率数据中解耦出缓慢变化的“基本面结构”和快速变化的“情绪结构”，我们采用了**绝热近似**：
1.  **快流形 (Fast Manifold)**：投资者的注意力查询向量 $Q_t \in \mathbb{R}^{d}$（情绪状态）随每日市场消息快速波动。
2.  **慢流形 (Slow Manifold)**：资产的内禀属性矩阵 $K \in \mathbb{R}^{N \times d}$（基本面特征）演化速度较慢。

假设市场中 $N$ 个资产的收益率向量 $R_t \in \mathbb{R}^N$ 由底层注意力权重驱动。我们通过在线非负矩阵分解 (Online NMF) 在长度为 $W$ 的滚动窗口中最小化以下重构误差，以动态求解这一逆问题：

$$
\min_{Q_t, K} \sum_{\tau=t-W+1}^{t} \left\| R_\tau - \text{Softmax}\left(\frac{Q_\tau K^T}{T}\right) V \right\|_F^2 + \lambda_Q \|Q_\tau\|_1 + \lambda_K \|K\|_F^2
$$

求解得到各个资产的注意力权重向量 $\mathbf{A}_t = \text{Softmax}(Q_t K^T / T) \in \mathbb{R}^N$ 后，我们利用**信息熵 (Shannon Entropy)** 来量化市场注意力的集中程度，并定义**拓扑脆弱性指数 (Topological Fragility Index, TFI)** 如下：

$$
TFI_t = 1 - \frac{\mathcal{H}(\mathbf{A}_t)}{\log(N)} = 1 + \frac{\sum_{i=1}^N A_{t,i} \log(A_{t,i})}{\log(N)}
$$

其中 $\mathcal{H}(\mathbf{A}_t)$ 为注意力分布的熵。
*   当注意力完全均匀分散时（正常市场），$A_{t,i} \approx 1/N$，熵最大，$\mathcal{H}(\mathbf{A}_t) = \log(N)$，此时 $TFI_t \approx 0$。
*   当注意力极端拥挤于少数资产时（危机前夕的隧道视野），分布趋于狄拉克函数 (Dirac Delta)，熵 $\mathcal{H}(\mathbf{A}_t) \to 0$，此时 $TFI_t \to 1$。

该指数以 $[0,1]$ 之间的连续区间，严格刻画了系统从高维发散到低维坍缩的拓扑相变过程。
...
在统计物理学中，微观主体的耦合强度 $J$ 与系统环境温度 $T$ 通常以 $J/T$ 的比例共同决定系统状态。在本文的金融物理映射中，**市场有效流动性 $T$ 的枯竭在微观动力学上等效于主体间社交传染强度 $J$ 的相对飙升**。

我们模拟了一个完整的“泡沫-崩盘”周期：随着社交压力参数 $J$ 从 0.1（独立决策）逐渐增加至 2.0（强烈的羊群效应），系统的拓扑结构发生了显著的相变（见图 2）。

![图 2: ABM 微观模拟](../figures/abm_simulation_proof.png)
*图 2: ABM 模拟结果。(A) 羊群效应强度 J 随时间变化；(B) 合成市场价格路径；(C) 拓扑脆弱性指数 (TFI)。注意红色区域（相变区）：在价格发生剧烈崩盘之前，TFI 已经因微观主体的同步化而提前飙升。*
...
### A.1 回测表现与适用边界探索 (China A-Shares & Boundary Conditions)
![图 A1: 中国市场回测结果](../figures/china_market_regime.png)
*图 A1: 中国 A 股市场实证回测。考虑到 A 股市场的高基底噪声与高波动性，我们采用了基于体制（Regime-Dependent）的自适应风控策略。*

实证结果显示：
*   **年化收益**: 策略 (14.71%) 低于基准 (18.09%)。
*   **波动率优化**: 年化波动率从 20.83% 降低至 18.35%。
*   **深层机理探讨 (Discussion on Boundary Conditions)**: 表面上看，策略在 A 股市场的表现不如美股市场显著（例如最大回撤出现倒挂：-44.64% vs -34.70%）。然而，这一结果恰好从侧面印证了 TFI 指标的理论根基。TFI 本质上捕捉的是**由微观注意力拥挤引发的内生流动性危机 (Endogenous Liquidity Crisis)**。而 A 股市场具有显著的“政策市”特征，其历史上的数次极端崩盘（如 2015 年骤然去杠杆、熔断机制等）多由**外生行政冲击 (Exogenous Regulatory Shock)** 直接触发。在外生冲击降临时，微观层面往往尚未经历充分的“铁磁有序化”蓄能阶段，系统就被外力直接击碎，导致 TFI 无法提前预警。这一**反事实验证 (Counter-factual Validation)** 清晰界定了 TFI 的物理学适用边界：它是探测内生结构性相变的雷达，而非预测外生黑天鹅的魔法。

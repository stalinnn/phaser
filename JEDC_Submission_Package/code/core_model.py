...
def download_data():
    ...
    # Batch download with chunking to avoid rate limits
    chunk_size = 5
    all_chunks = []
    ...
    try:
        # Concatenate chunks (assuming columns are MultiIndex [Price, Ticker])
        # If single level, adjust logic
        ...
        # FIX: Increased threshold from 0.7 to 0.95 to ensure we keep 2000-2006 data.
        # If we include stocks that listed in 2006 (e.g. V, MA), dropna() cuts off the early history.
        ...
        if volume is not None:
             # Ensure we only keep volumes for the closes we kept
...
class AdiabaticAttentionV2:
    ...
    def fit_step(self, returns_window):
        ...
        # Temperature Scaling
        # Optimized Physics-Informed Temperature (Matching Backtest Logic)
        # T -> 0 as Correlation -> 1 (Phase Transition)
...
def run_v2():
    ...
    if returns.empty:
        print("Not enough data to calculate returns. Aborting.")
        return

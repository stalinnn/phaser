...
def run_criticality_search():
    ...
    # 1. Order Parameter (R)
    plt.subplot(1, 2, 1)
    plt.plot(Ks, results_order, 'o-', color='#27ae60', linewidth=2, markersize=8)
    
    # Highlight the dip at K=1
    plt.plot(1, results_order[1], 'ro', markersize=10, label='Instability (K=1)')
    plt.annotate('Overshoot\nInstability', xy=(1, results_order[1]), xytext=(2.5, results_order[1]-0.1),
    ...
    plt.xlabel('Coordination Depth (K)')
    plt.ylabel('Order Parameter R')
    plt.title('Consensus Quality')
    plt.grid(True, alpha=0.3)
    plt.legend()
    
    # 2. System Energy (Log Scale)
    plt.subplot(1, 2, 2)
    plt.plot(Ks, results_energy, 's-', color='#e74c3c', linewidth=2, markersize=8)
    plt.yscale('log')
    
    plt.xlabel('Coordination Depth (K)')
    plt.ylabel('System Energy (Log Scale)')
    plt.title('Thermodynamic Dissipation')
    plt.grid(True, alpha=0.3)
    
    plt.suptitle('Non-monotonic Phase Transition in Distributed Coordination', fontsize=14)
    plt.tight_layout()
    plt.savefig('figures/criticality_search.png', dpi=300)
    print("Saved figure to figures/criticality_search.png")

...
def main():
    ...
    # Plot 1
    ax1.plot(scalar_trajectories, label='Scalar', color='#e74c3c', alpha=0.8)
    ax1.plot(tensor_trajectories, label='Distributed Tensor (K=20)', color='#2ecc71')
    ax1.set_title(f'Dynamics at High Entropy (T=2.5)', fontsize=12)
    ax1.set_yscale('log')
    ax1.legend()
    
    # Plot 2
    ax2.errorbar(temperatures, scalar_final_errors, yerr=scalar_std, fmt='o-', label='Scalar', color='#e74c3c')
    ax2.errorbar(temperatures, tensor_final_errors, yerr=tensor_std, fmt='s-', label='Distributed Tensor', color='#2ecc71')
    
    ax2.fill_between(temperatures, scalar_final_errors, tensor_final_errors, color='#2ecc71', alpha=0.15)
    
    ax2.set_title('Thermodynamic Stability (Honest)', fontsize=12)
    ...
    print("Saved to figures/first_principles_network_dynamics_fixed.png")

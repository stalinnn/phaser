...
def main():
    ...
    # Don't wrap DDP yet! Wait until weights are loaded.
    ...
    elif args.resume_ckpt != 'none':
        ckpt_path = args.resume_ckpt

    if ckpt_path and os.path.exists(ckpt_path):
        if master_process: print(f"Resuming from {ckpt_path} with NEW LAMBDA={args.sparsity_penalty}...")
        # Load weights into BASE model (before DDP)
        state_dict = torch.load(ckpt_path, map_location=device)
        model.load_state_dict(state_dict)
        start_step = 0 # NEW LOGIC: Always start from 0 for hotfix phase
        if master_process: print(f"Loaded weights. Resetting step to 0.")
    else:
    ...
    # Log Append
    if master_process:
        # Use lambda in filename to distinguish experiments
        log_name = f'training_log_hotfix_lambda_{args.sparsity_penalty}.csv'
        csv_file = open(os.path.join(args.out_dir, log_name), 'w')
    ...
    while step < args.steps:
        ...
        if master_process:
            ...
            pbar.update(1)
            if step % 1000 == 0: 
                ckpt_name = f'ckpt_{step}_hotfix_lambda_{args.sparsity_penalty}.pt'
                torch.save(raw_model.state_dict(), os.path.join(args.out_dir, ckpt_name))

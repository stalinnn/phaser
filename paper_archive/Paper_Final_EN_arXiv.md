...
To explain the “insight-like” sparsification observed in experiments (gate rate drops sharply after accuracy reaches 100%), we adopt a control-theoretic view. The total objective is

$$
\mathcal{L}_{total} = \mathcal{L}_{task}(\theta) + \lambda(\alpha)\cdot \|g\|_1,
$$

where \(\lambda(\alpha)=\lambda_{max}\cdot \alpha^\gamma\) adapts with accuracy \(\alpha\) (we use \(\gamma=3\)). This induces two distinct dynamical phases:

1. **Adiabatic learning phase (\(\alpha \ll 1 \Rightarrow \lambda \approx 0\))**: unconstrained optimization dominated by \(\nabla \mathcal{L}_{task}\). Since attention provides more efficient gradient transport than RNN, the system tends to open the gate to rapidly reduce error.
2. **Isothermal compression phase (\(\alpha \to 1 \Rightarrow \lambda \to \lambda_{max}\))**: once the task is solved, \(\nabla \mathcal{L}_{task}\to 0\) and the regularizer dominates, producing sustained “thermodynamic pressure” that shrinks \(g\) toward zero while preserving accuracy.

Mathematically, this resembles **homotopy continuation**: the system first finds a global solution on a flatter energy surface, then as the penalty “gravity” increases, the solution trajectory falls into the lowest-entropy sparse basin. The sharp gate-rate drop observed around Iter 1900 corresponds to a phase-transition-like jump from a wide/shallow dense solution to a narrow/deep optimal sparse solution.
